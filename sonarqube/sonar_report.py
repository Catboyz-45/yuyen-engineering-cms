...
# ---------------------------------------------------------------------------
# ฟอนต์ภาษาไทย (เพิ่มจากต้นฉบับ: ฟอนต์ Helvetica ของ reportlab ไม่มีอักษรไทย หัวข้อภาษาไทยจึงกลายเป็นกล่องว่าง)
# ---------------------------------------------------------------------------
...
def register_thai_font(font_path=None):
    """ลงทะเบียนฟอนต์ที่มีอักษรไทย คืนชื่อ (ตัวปกติ, ตัวหนา); หาไม่เจอจะใช้ Helvetica ตามต้นฉบับ"""
    candidates = [(font_path, font_path)] if font_path else THAI_FONT_CANDIDATES
    for regular, bold in candidates:
        if regular and os.path.exists(regular):
            pdfmetrics.registerFont(TTFont("Thai", regular))
            pdfmetrics.registerFont(TTFont("Thai-Bold", bold if bold and os.path.exists(bold) else regular))
            return "Thai", "Thai-Bold"
    print("ไม่พบฟอนต์ภาษาไทย ข้อความภาษาไทยใน PDF อาจแสดงไม่ได้ (ระบุด้วย --font)", file=sys.stderr)
...
GATE_LABEL = {"OK": "PASSED", "ERROR": "FAILED"}
...
def fetch_issues_by_severity(host, token, project_key):
    ...
    for sev in SEVERITY_ORDER:
        ...
        params = {
            ...
            "severities": sev,
            "resolved": "false",  # เพิ่มจากต้นฉบับ: นับเฉพาะที่ยังไม่แก้ (สแกนซ้ำแล้ว issue ที่แก้จะเป็น CLOSED)
...
def build_pdf(output_path, project_name, project_key, host, measures, sev_counts, top_issues, font_path=None):
    doc = SimpleDocTemplate(
        output_path, pagesize=A4,
        topMargin=1.5 * cm, bottomMargin=1.5 * cm,
        leftMargin=1.5 * cm, rightMargin=1.5 * cm,
    )
    font, font_bold = register_thai_font(font_path)
    styles = getSampleStyleSheet()
    for style in styles.byName.values():
        style.fontName = font_bold if getattr(style, "fontName", "").endswith("Bold") else font
    ...
    gate_label = GATE_LABEL.get(gate_status, gate_status)
    ...
    metric_table.setStyle(TableStyle(font_rules + [
    ...
    sev_table.setStyle(TableStyle(font_rules + [
        ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#3b3e99")),
        ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
        ("FONTSIZE", (0, 0), (-1, -1), 10),
        ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
    ...
    for issue in top_issues:
        ...
        issue_rows.append([
            issue.get("severity", "-"),
            issue.get("type", "-"),
            Paragraph(escape(component), cell_style),
            Paragraph(escape(message), cell_style),
        ])
    issue_table = Table(issue_rows, colWidths=[2.2 * cm, 2.5 * cm, 5 * cm, 7.3 * cm])
    issue_table.setStyle(TableStyle(font_rules + [
        ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#3b3e99")),
        ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
        ("FONTSIZE", (0, 0), (-1, -1), 8),
        ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
...
def main():
    ...
    parser.add_argument("--font", default=None, help="ไฟล์ฟอนต์ .ttf ที่มีอักษรไทย (default: หาอัตโนมัติ)")
    ...
    try:
        ...
        build_pdf(output, project_name, args.project, host, measures, sev_counts, top_issues, args.font)
