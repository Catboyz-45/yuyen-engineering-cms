...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await audit({ actorId: session.adminId, action: "ADMIN_TWO_FACTOR_RESET", targetType: "Admin", targetId: id, result: "SUCCESS", ...context });
    return NextResponse.json({ success: true });
  } catch (error) { await audit({ actorId: session.adminId, action: "ADMIN_TWO_FACTOR_RESET", targetType: "Admin", targetId: id, result: "FAILURE", ...context }); return cmsError(error, context.requestId); }
}
