...
export default async function ProjectDetail({ params }: Readonly<{ params: Promise<{ slug: string }> }>) { const { slug } = await params; const item = await content.getProject(slug); if (!item) { await redirectOldSlug(`/projects/${slug}`, value => content.getProject(value)); notFound(); } return <><section className="page-hero"><div className="container"><DetailNav href="/projects" rootLabel="ผลงาน" trail={[item.title]} /><h1 className="display">{item.title}</h1><div className="detail-meta"><span className="tag">{item.projectType}</span><span className="cluster"><MapPin size={18} /> {item.area}</span>{item.completedAt && <span className="cluster"><CalendarDays size={18} /> {formatThaiDate(item.completedAt)}</span>}</div></div></section><section className="section"><div className="container"><div className="detail-grid"><MediaGallery images={galleryImages(item.coverMedia, item.gallery)} label={`รูปผลงาน ${item.title}`} fallbackClass="office" /><div><h2 className="heading">ภาพรวมโครงการ</h2><p className="lead">{item.summary}</p><div style={{ whiteSpace: "pre-wrap" }}>{item.content}</div>{item.showCustomerName && item.customerName && <p>ลูกค้า: {item.customerName}</p>}{item.services.length > 0 && <div className="related-services"><h3>บริการที่เกี่ยวข้อง</h3><ul className="related-links">{item.services.map(({ service }) => <li key={service.slug}><Link href={`/services/${service.slug}`}>{service.title}<ArrowRight size={15} aria-hidden="true" /></Link></li>)}</ul></div>}</div></div></div></section></>; }
