...
export default async function ServiceDetail({ params }: Readonly<{ params: Promise<{ slug: string }> }>) { const { slug } = await params; const item = await content.getService(slug); if (!item) { await redirectOldSlug(`/services/${slug}`, value => content.getService(value)); notFound(); } return <><section className="page-hero"><div className="container"><DetailNav href="/services" rootLabel="บริการ" trail={[item.title]} /><h1 className="display">{item.title}</h1><p className="lead">{item.summary}</p></div></section><section className="section"><div className="container detail-grid"><div><h2 className="heading">รายละเอียดบริการ</h2><div className="lead" style={{ whiteSpace: "pre-wrap" }}>{item.content ?? item.summary}</div><div className="cluster" style={{ marginTop: 34 }}><Link className="btn btn-primary" href="/contact"><Phone size={17} /> ติดต่อสอบถาม</Link></div></div><MediaGallery images={galleryImages(item.coverMedia, item.gallery)} label={`รูปบริการ ${item.title}`} fallbackClass="building" /></div></section></>; }
