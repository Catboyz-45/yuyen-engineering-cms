...
export default async function ServicesPage() { const content = new PublicContentService(); const [records, company] = await Promise.all([content.listServices(), content.getCompany()]); const copy = resolveSiteCopy(company?.siteCopy); const hasPhotos = records.every(item => item.coverMedia); const items = records.map(item => ({ slug: item.slug, title: item.title, eyebrow: item.eyebrow ?? "SERVICE", description: item.summary, icon: resolveServiceIcon(item), media: hasPhotos ? item.coverMedia : null })); return <><section className="page-hero"><div className="container"><p className="eyebrow">OUR SERVICES</p><h1 className="display">บริการของเรา</h1>{copy.servicesIntro && <p className="lead">{copy.servicesIntro}</p>}</div></section><section className="section"><div className="container"><div className="grid-2">{items.map(item => <ServiceCard key={item.slug} item={item} />)}</div>{!items.length && <p className="muted">ยังไม่มีบริการที่เผยแพร่</p>}</div></section></>; }
