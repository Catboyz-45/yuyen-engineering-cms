...
export function AdminAccounts() {
  ...
  const loadUsers = useCallback(async () => {
    try {
      ...
      setUsers(liveUsers(result)); setCurrentUserId(result.currentUserId);
  ...
  async function submitUser(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (dialog?.type === "edit" && dialog.user) {
        ...
        await api(`/api/admin/users/${dialog.user.id}`, { method: "PATCH", body: JSON.stringify({ displayName, ...(role !== dialog.user.role ? { role } : {}) }) });
