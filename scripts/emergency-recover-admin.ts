...
// Mirrors authThrottleKey in src/server/auth/throttle.ts ("<budget>:account:<subject>"), which cannot be imported outside Next.js.
...
async function main() {
  ...
  try {
    ...
    if (user?.role !== "SUPER_ADMIN") throw new Error("Super Admin account not found");
    ...
    await db.$transaction([
      ...
      ...(process.env.SESSION_SECRET ? [db.authThrottle.deleteMany({ where: { key: { in: [throttleKey(`login:account:${user.usernameNormalized}`, process.env.SESSION_SECRET), throttleKey(`second-factor:account:${user.id}`, process.env.SESSION_SECRET)] } } })] : []),
